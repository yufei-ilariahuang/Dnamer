...
static char OFFSET;
static char NEW_X;
static char SPACE_INDEX;

// Desktop names loaded from plist
static NSMutableArray *desktopNamesArray = nil;
static const int MAX_DESKTOPS = 16;

// Track space ordering during Mission Control
static NSMutableArray *currentSpaceOrder = nil;
static BOOL isInMissionControl = NO;
...
static void loadDesktopNames(void) {
    ...
    NSArray *loadedArray = [[NSArray alloc] initWithContentsOfFile:plistPath];
    
    if (loadedArray && [loadedArray isKindOfClass:[NSArray class]]) {
        desktopNamesArray = [loadedArray mutableCopy];
        NSLog(@"✅ Loaded desktop names from plist: %@", desktopNamesArray);
    } else {
        NSLog(@"⚠️  No desktop names plist found or invalid format, using defaults");
        desktopNamesArray = [@[] mutableCopy];
    }
}

// Helper: Get desktop name by number (1-based index)
static NSString *getDesktopName(int desktopNumber) {
    if (!desktopNamesArray) {
        loadDesktopNames();
    }
    
    // Convert 1-based desktop number to 0-based array index
    int index = desktopNumber - 1;
    
    // Return custom name if exists at this index, otherwise return default
    if (index >= 0 && index < desktopNamesArray.count) {
        NSString *name = desktopNamesArray[index];
        if (name && [name isKindOfClass:[NSString class]] && name.length > 0) {
            return name;
        }
    }
    
    return [NSString stringWithFormat:@"Desktop %d", desktopNumber];
}
...
// Helper: Tag a layer with its space index
static void tagSpaceIndex(CALayer *view, int spaceIndex) {
    CATextLayer *textLayer = getTextLayer(view);
    if (!textLayer) return;
    
    objc_setAssociatedObject(textLayer, &SPACE_INDEX, @(spaceIndex), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    
    CALayer *parent = textLayer.superlayer;
    if (parent) {
        objc_setAssociatedObject(parent, &SPACE_INDEX, @(spaceIndex), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        
        // Tag siblings
        for (CALayer *sibling in parent.sublayers) {
            objc_setAssociatedObject(sibling, &SPACE_INDEX, @(spaceIndex), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
    }
    
    NSLog(@"🏷️  Tagged layer with space index: %d", spaceIndex);
}
    ...
    if (frame.origin.x == 0 && [self.superlayer.class isEqual:[CALayer class]]) {
        ...
        if (root.sublayers.count > 0) {
            ...
            if (container.sublayers.count >= 2) {
                ...
                NSLog(@"📊 Found %lu space views (unexpanded), %lu expanded",
                      (unsigned long)unexpandedViews.count, (unsigned long)expandedViews.count);
                
                // Apply custom names to ALL spaces (regardless of type)
                double unexpandedOffset = 0;
                
                for (int i = 0; i < MIN(unexpandedViews.count, MAX_DESKTOPS); i++) {
                    NSString *name = getDesktopName(i + 1);  // Position-based naming (1-indexed)
                    
                    // Unexpanded view
                    if (i < unexpandedViews.count) {
                        CALayer *space = unexpandedViews[i];
                        CATextLayer *textLayer = getTextLayer(space);
                        if (textLayer) {
                            double originalWidth = textLayer.bounds.size.width;
                            double newWidth = getTextSize(textLayer, name);
                            overrideTextLayer(space, name, newWidth);
                            tagSpaceIndex(space, i);  // Tag with position index
                            setOffset(space, unexpandedOffset, NO);
                    ...
                    if (i < expandedViews.count) {
                        CALayer *space = expandedViews[i];
                        CATextLayer *textLayer = getTextLayer(space);
                        if (textLayer) {
                            double newWidth = getTextSize(textLayer, name);
                            // Limit to parent width
                            newWidth = MIN(newWidth, space.frame.size.width);
                            overrideTextLayer(space, name, newWidth);
                            tagSpaceIndex(space, i);  // Tag with position index
                        }
                    }
...
- (void)setString:(id)string {
    ...
    if (!currentSpaceOrder) {
        currentSpaceOrder = [[NSMutableArray alloc] init];
    }
    
    // Detect Mission Control opening (multiple space names being set rapidly)
    static NSTimeInterval lastSetStringTime = 0;
    NSTimeInterval now = [[NSDate date] timeIntervalSince1970];
    
    if (now - lastSetStringTime > 0.5) {
        // New batch - likely Mission Control opened
        [currentSpaceOrder removeAllObjects];
        isInMissionControl = YES;
        NSLog(@"🎯 Mission Control detected - resetting space order");
    }
    lastSetStringTime = now;
    
    // Check if this looks like a space name (Desktop X, app names, etc.)
    BOOL isSpaceName = NO;
    int spacePosition = -1;
    ...
    if ([stringValue hasPrefix:@"Desktop "] ||
        ([stringValue length] > 0 &&
         ...
         ![stringValue containsString:@"desktop_renamer"] &&
         ![stringValue isEqualToString:@""] &&
         ![stringValue isEqualToString:@"liahuang"])) {  // Filter out non-space UI elements
        
        isSpaceName = YES;
        
        // Check if we've seen this space already in this batch
        NSNumber *existingIndex = [currentSpaceOrder indexOfObject:stringValue] != NSNotFound
            ? @([currentSpaceOrder indexOfObject:stringValue])
            : nil;
        
        if (existingIndex) {
            spacePosition = [existingIndex intValue];
            NSLog(@"🔄 Duplicate space '%@' at position %d", stringValue, spacePosition);
        } else {
            // New space in this batch
            [currentSpaceOrder addObject:stringValue];
            spacePosition = (int)currentSpaceOrder.count - 1;
            NSLog(@"📍 New space '%@' assigned position %d", stringValue, spacePosition);
        }
        
        // Get custom name for this position (1-based)
        if (spacePosition >= 0 && spacePosition < MAX_DESKTOPS) {
            NSString *customName = getDesktopName(spacePosition + 1);
            if (customName) {
                string = customName;
                NSLog(@"✅ Renamed space at position %d: '%@' → '%@'", spacePosition, stringValue, customName);
                
                // Tag this layer with the space index for future reference
                objc_setAssociatedObject(self, &SPACE_INDEX, @(spacePosition), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
            }
        }
    }
